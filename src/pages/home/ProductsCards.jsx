...
const CollaborationSection = () => {
   ...
   return (
      <section className="py-8 bg-gradient-to-b from-slate-50 via-blue-50 to-indigo-100">
         <div className="max-w-9xl mx-auto px-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
               {collaborationProducts.map((product) => (
                  <div
                     key={product.id}
                     className="group relative flex flex-col rounded-3xl overflow-hidden bg-white shadow-xl hover:shadow-2xl backdrop-blur-sm transition-all duration-500 transform hover:-translate-y-3 hover:scale-102"
                  >
                     {/* Product Image */}
                     <div className="relative overflow-hidden aspect-[4/5]">
                        ...
                        >
                           <Heart
                              className={`w-5 h-5 ${favorites.has(product.id) ? "fill-red-500 text-red-500" : "text-slate-400"
                                 }`}
                           />
                        </button>
                     </div>
