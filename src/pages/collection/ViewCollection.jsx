...
const ViewCollection = () => {
  ...
  // ── Real wishlist from context (was local state before — that was the bug) ──
  ...
  return (
    <div className="min-h-screen" style={{ background: "linear-gradient(135deg, #0f172a 0%, #1e293b 50%, #0f172a 100%)" }}>
      ...
      {/* ── MARQUEE ── */}
      <div style={{ background: "rgba(0,0,0,0.4)", borderBottom: "1px solid rgba(255,255,255,0.07)", overflow: "hidden", padding: "10px 0" }}>
        <div style={{ display: "flex", width: "max-content", animation: "marquee 28s linear infinite" }}>
          {[...Array(4)].map((_, i) => (
            <span key={i} style={{ fontSize: 10, letterSpacing: "0.28em", textTransform: "uppercase", color: "#ffffff", whiteSpace: "nowrap", fontWeight: 500 }}>
              {MARQUEE_TEXT}
            </span>
          ))}
        </div>
      ...
      {/* ── HERO ── */}
      <div style={{ padding: "48px 24px 40px", textAlign: "center", background: "linear-gradient(135deg, #0f172a 0%, #1e293b 50%, #0f172a 100%)" }}>
        <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 999, padding: "7px 18px", marginBottom: 20 }}>
          <span style={{ fontSize: 13, color: "rgba(255,255,255,0.75)", letterSpacing: "0.04em" }}>✦  Fall / Winter 2024 - New Collection</span>
        </div>
        <h1 style={{ fontSize: "clamp(36px, 5.5vw, 64px)", fontWeight: 700, color: "#fff", lineHeight: 1.05, margin: "0 0 14px", letterSpacing: "-0.03em" }}>
          New Collection &{" "}
          <span style={{ background: "linear-gradient(90deg, #f59e0b, #f97316)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
            Luxury Redefined
          </span>
        </h1>
        <p style={{ fontSize: 16, color: "rgba(255,255,255,0.45)", maxWidth: 480, margin: "0 auto", lineHeight: 1.7, fontWeight: 300 }}>
          Designs crafted with precision — made for those who value subtle luxury and refined aesthetics.
        </p>
      </div>

      {/* ── FILTER BAR ── */}
      <div style={{ position: "sticky", top: 0, zIndex: 40, background: "rgba(15,23,42,0.88)", backdropFilter: "blur(20px)", borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <div style={{ position: "relative" }}>
                  ...
                  <ChevronDown size={13} style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", color: "rgba(255,255,255,0.45)", pointerEvents: "none" }} />
                </div>
              </div>
            </div>
          </div>
        ...
        {viewMode === "grid" && (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 20, alignItems: "stretch" }}>
            {filtered.map((p, i) => {
              ...
              const wishlisted = isWishlisted(p.id); // ← real global state
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    boxShadow: hovered === p.id ? "0 20px 60px rgba(0,0,0,0.5)" : "0 4px 16px rgba(0,0,0,0.2)",
                  }}
                ...
                >
                  ...
                  <div style={{ position: "relative", aspectRatio: "3/4", overflow: "hidden", background: "rgba(255,255,255,0.03)" }}>
                    <img src={img} alt={p.name} className="col-card-img" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
                    ...
                    {/* ── WISHLIST HEART BUTTON — now connected to global context ── */}
                    <button
                      className="heart-btn"
                      onClick={(e) => {
                        e.stopPropagation(); // prevent card navigation
                        toggleWishlist(p);   // ← real toggle
                      }}
                      style={{
                        position: "absolute", top: 12, right: 12,
                        background: wishlisted ? "rgba(239,68,68,0.15)" : "rgba(15,23,42,0.7)",
                  ...
                  <div style={{ padding: "16px 18px 20px" }}>
                    <h3 style={{ fontSize: 15, fontWeight: 600, color: "#fff", margin: "0 0 4px", lineHeight: 1.3 }}>{p.name}</h3>
                    <p style={{ fontSize: 11, letterSpacing: "0.15em", textTransform: "uppercase", color: "rgba(255,255,255,0.4)", margin: "0 0 14px" }}>
                      {p.category}
                    </p>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                      <span style={{ fontSize: 20, fontWeight: 700, color: "#fff" }}>{p.price}</span>
                      <span style={{ fontSize: 11, letterSpacing: "0.12em", textTransform: "uppercase", color: "rgba(255,255,255,0.4)" }}>View →</span>
                    </div>
                  </div>
                </div>
              );
              ...
              return (
                ...
                >
                  ...
                  <div style={{ flex: 1, padding: "20px 24px", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
                    <div>
                      <p style={{ fontSize: 10, letterSpacing: "0.2em", textTransform: "uppercase", color: "rgba(255,255,255,0.3)", margin: "0 0 6px" }}>{p.category}</p>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                      ...
                      <span style={{ fontSize: 11, letterSpacing: "0.12em", textTransform: "uppercase", color: "rgba(255,255,255,0.35)" }}>View Details →</span>
                      {/* Heart in list mode too */}
                      <button
                        className="heart-btn"
                      ...
                      >
                        <Heart size={14} fill={wishlisted ? "#ef4444" : "none"} color={wishlisted ? "#ef4444" : "rgba(255,255,255,0.4)"} />
      ...
      <div className="mx-auto text-center pb-10">
        <button className="bg-blue-950 px-6 py-3 rounded-xl font-semibold hover:bg-blue-800 text-2xl text-white">
          View More Product
        </button>
      </div>
    </div>
